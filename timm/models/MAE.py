...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, mask=None):
        ...
        if mask != None:
            ...
            # if self.dist_token is None:
            #     return self.pre_logits(x_masked[:, 0])
            # else:
            #     return x_masked[:, 0], x_masked[:, 1]
        else:
            x = self.blocks(x)
            x = self.norm(x)
...
class MAE(nn.Module):
    def __init__(self):
        ...
        # encoder = vit_base_patch16_384(pretrained=True)
    ...
    def forward(self, x, mask):
        ...
        pos_embed = get_sinusoid_encoding_table(576, 768)

        B, N, C = x.shape

        expand_pos_embed = pos_embed.expand(B, -1, -1).type_as(x).clone().detach()
        
        expand_pos_embed[~mask,:] += x.reshape(B*N, C)
        output = self.decoder(expand_pos_embed)
